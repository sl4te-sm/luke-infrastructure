...
CERT_DIR=/home/etcd/.ssl
CRL=/mnt/servershare/intermediate.crl.pem
...
if ! [ -f ${CERT_DIR}/ca.crt ]; then
    ...
    exit 1
elif ! [ -f ${CERT_DIR}/${HOST_NAME}.luke-domain.com.crt ]; then
    echo "Missing host certificate"
    exit 1
elif ! [ -f ${CERT_DIR}/${HOST_NAME}.luke-domain.com.key ]; then
...
ExecStart=/usr/local/bin/etcd \\
    ...
    --client-cert-auth --trusted-ca-file=${CERT_DIR}/ca.crt --client-crl-file=${CRL} \\
    --cert-file=${CERT_DIR}/${HOST_NAME}.luke-domain.com.crt \\
    --key-file=${CERT_DIR}/${HOST_NAME}.luke-domain.com.key \\
    --peer-client-cert-auth --peer-trusted-ca-file=${CERT_DIR}/ca.crt --peer-crl-file=${CRL} \\
    --peer-cert-file=${CERT_DIR}/${HOST_NAME}.luke-domain.com.crt \\
    --peer-key-file=${CERT_DIR}/${HOST_NAME}.luke-domain.com.key
